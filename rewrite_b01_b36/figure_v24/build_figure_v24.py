#!/usr/bin/env python3
"""Build Figure v2.4 hybrid SVG/PDF/PNG assets.

v2.4 cuts text density, enlarges type, and fixes connector-label overlap. It builds on v2.3 which restyled v2.2 after the GPT-image design references: vector icons, lifted dark renders, lane labels. It keeps the empirical content and B01--B36 provenance from figure_v2 while
...
def build_fig1():
    ...
    svg += [rect(75,340,80,66,"#f8fafc",GRID,1.8,5),
            ...
            text("registered geometry, imagery, records, prior state",230,470,340,18,fill=MUTED)]

    bullets2=[
        ("doc", "select evidence / domain"),
        ("layers", "choose the representation"),
        ("branch", "decompose the task"),
        ("code", "write / revise the program"),
        ("chat", "interpret validator feedback"),
    ]
    for i,(ic,s) in enumerate(bullets2):
        yy=196+i*56
        svg.append(icon(ic,492,yy-6,13,PURPLE))
        svg.append(text(s,526,yy,250,20,anchor="start",family=SANS))

    bullets3=[
        ("sliders", "fit / transform / spline"),
        ("cube", "construct geometry in Blender"),
        ("image", "render fixed views"),
        ("check-square", "endpoint / contact checks"),
        ("shield", "coverage / collision / preservation"),
    ]
    for i,(ic,s) in enumerate(bullets3):
        yy=196+i*56
        svg.append(icon(ic,864,yy-6,13,ORANGE))
        svg.append(text(s,900,yy,290,20,anchor="start",family=SANS))
    ...
    svg += [image(1262,170,360,246,station),
            text("accepted components, connections, and provenance become the next batch's context",1443,456,335,18,fill=MUTED)]
    ...
    bottom=[
        (30,674,372,242,"Local fit","comparison domain, pose, primitives",SOFT_BLUE,BLUE,hist("installation_B01/B01_front_overlay.jpg")),
        (430,674,372,242,"Reusable scope","shared MASTER vs site-specific geometry",SOFT_PURPLE,PURPLE,hist("installation_B08/42_B08_Transformer_Pair_r2.png")),
        (830,674,372,242,"Connected representation","ports, routes, continuity, path class",SOFT_ORANGE,ORANGE,hist("installation_B23/r3/480_B23_T1_Neutral_Oblique_Overlay.png")),
        (1230,674,410,242,"State-constrained integration","coverage, review, preservation, interference",SOFT_TEAL,TEAL,hist("installation_B36/r2_previews/748_B36_T1_Rack_Overlay.png")),
    ]
    for x,y,w,h,t,foot,tint,stroke,p in bottom:
        ...
        svg.append(text(foot,x+w/2,y+216,w-34,18,fill=MUTED))
    svg += [arrow(402,794,428,794),arrow(802,794,828,794),arrow(1202,794,1228,794)]
    svg.append('</svg>')
    (SVG_DIR/"fig01_overview_v24.svg").write_text("".join(svg),encoding="utf-8")
...
def build_fig4():
    ...
    rows=[
        (110,"A · Fit → reusable scope",BLUE,[
            ("B01: local fit","define a comparison domain; fit pose and size",hist("installation_B01/B01_front_overlay.jpg"),SOFT_BLUE,BLUE),
         ...
         ["repetition → invariance","reuse boundary testable"]),
        (426,"B · Object → connected representation",PURPLE,[
            ...
            ("B23: change representation","connection reformulated: straight → curved",hist("installation_B23/B23_measurement_profiles.png"),SOFT_ORANGE,ORANGE),
            ("B29: relational constraints","continuity constraints scale across subsystems",hist("installation_B29/B29_crossline_fits.png"),SOFT_TEAL,TEAL)],
         ["relationships appear","endpoint constraints scale"]),
        (742,"C · Addition → state-driven closure",TEAL,[
            ...
            ("B26: reconstruct selected gaps","coverage selects what to reconstruct next",hist("installation_B26/543_B26_Structure_Overlay.png"),SOFT_PURPLE,PURPLE),
            ("B36: state-constrained insertion","new geometry must fit inherited endpoints",hist("installation_B36/B36_busbar_plan_overlay.png"),SOFT_TEAL,TEAL)],
         ["coverage → task selection","fit the inherited world"]),
    ]
    panel_x=[176,600,1024]
    panel_w=330
    for r,(y,row_title,row_stroke,cards,links) in enumerate(rows):
        ...
        for c,(title_s,foot,p,tint,stroke) in enumerate(cards):
            ...
            svg.append(text(foot,x+panel_w/2,y+242,panel_w-30,18,fill=MUTED))
        # Thin paper-style connectors. Labels float above the line, inside the gap.
        x1a, x2a = panel_x[0]+panel_w, panel_x[1]-4
        svg.append(arrow(x1a,y+150,x2a,y+150,sw=2.1))
        svg.append(text(links[0],(x1a+x2a)/2,y+126,90,15,"700",fill=MUTED))
        x1b, x2b = panel_x[1]+panel_w, panel_x[2]-4
        svg.append(arrow(x1b,y+150,x2b,y+150,sw=2.1))
        svg.append(text(links[1],(x1b+x2b)/2,y+126,90,15,"700",fill=MUTED))
        if r<2:
    ...
    svg.append('</svg>')
    (SVG_DIR/"fig04_transitions_v24.svg").write_text("".join(svg),encoding="utf-8")
...
def build_fig5():
    ...
    for x,y,w,bid,sub,foot,p,tint,stroke in top:
        ...
        svg.append(text(sub,x+w/2,y+80,w-30,18,"700",fill=BODY))
        svg.append(image(x+16,y+116,w-32,198,embed(p,1100,True,True)))
        svg.append(text(foot,x+w/2,y+372,w-26,17,fill=MUTED))

    for x1,label,x2 in [
        (310,"reuses B08 terminal",394),
        (684,"extends endpoint graph",768),
        (1058,"closes onto stubs",1142),
    ]:
        svg.append(arrow(x1,324,x2-3,324,sw=2.1))
        svg.append(text(label,(x1+x2)/2,290,82,14,"700",fill=MUTED,line_height=1.12))
    ...
    risk=("An early reusable representation gave multiple GIS installations the same finite bus-spool extent; "
          "sites with different separations overlapped. Recovery kept reusable equipment shared and moved "
          "finite pipe sections and supports to site scope.")
    svg.append(text(risk,430,690,390,19,anchor="start",fill=BODY))
    ...
    svg.append('</svg>')
    (SVG_DIR/"fig05_persistence_v24.svg").write_text("".join(svg),encoding="utf-8")


def export():
    figs=["fig01_overview_v24","fig04_transitions_v24","fig05_persistence_v24"]
    for stem in figs:
