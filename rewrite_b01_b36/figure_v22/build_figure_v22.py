#!/usr/bin/env python3
"""Build Figure v2.2 hybrid SVG/PDF/PNG assets.

v2.2 keeps the empirical content and B01--B36 provenance from figure_v2 while
...
def build_fig1():
    ...
    panel(svg,*p1,"1. Physical evidence + prior state",SOFT_BLUE,BLUE,21,54)
    ...
    for i,s in enumerate(bullets2):
        ...
        svg.append(text(s,528,yy,245,18,anchor="start",family=SANS))
    ...
    for x,y,w,h,t,foot,tint,stroke,p in bottom:
        ...
        svg.append(image(x+18,y+60,w-36,126,embed(p,1100,True)))
    ...
    svg.append('</svg>')
    (SVG_DIR/"fig01_overview_v22.svg").write_text("".join(svg),encoding="utf-8")
...
def build_fig4():
    ...
    for r,(y,row_title,row_stroke,cards,links) in enumerate(rows):
        ...
        pill(svg,20,y+108,132,54,row_title,
        ...
        for c,(title_s,foot,p,tint,stroke) in enumerate(cards):
            ...
            svg.append(image(x+20,y+64,panel_w-40,146,embed(p,1100,True)))
    ...
    svg.append('</svg>')
    (SVG_DIR/"fig04_transitions_v22.svg").write_text("".join(svg),encoding="utf-8")
...
def build_fig5():
    ...
    for x,y,w,bid,sub,foot,p,tint,stroke in top:
        ...
        svg.append(image(x+16,y+116,w-32,198,embed(p,1100,True)))
    ...
    svg.append('</svg>')
    (SVG_DIR/"fig05_persistence_v22.svg").write_text("".join(svg),encoding="utf-8")


def export():
    figs=["fig01_overview_v22","fig04_transitions_v22","fig05_persistence_v22"]
    for stem in figs:
